...
$.get(viewData.path, function (data) {
    var html = '';
    if (data.usersectionroles.length > 0) {
        ...
        data.sections_taking.forEach((section, sectionIndex) => {
            ...
            html +=             `<a href="{{path('section', {'sectionId':section.id})}}">${section.course.code}</a>
                            </div>`;
            section.assignments.forEach((assignment, assignmentIndex) => {
                ...
                html +=             `<a href="{{path('assignment', {'sectionId':assignment.section.id, 'assignmentId':assignment.id})}}">${assignment.name}</a>
                                </div>`;
                assignment.problems.forEach((problem, problemIndex) => {
                    html +=     `<div class="nav-problem assignment-${assignment.id}"><a href="{{path('assignment', {'sectionId':assignment.section.id, 'assignmentId':assignment.id, 'problemId':problem.id})}}">${problem.name}</a></div>`;
                });
                html +=     `</div>`;
            });
            html +=     `</div>
    ...
    if (data.sections_teaching.length > 0) {
